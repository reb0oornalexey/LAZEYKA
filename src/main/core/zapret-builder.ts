...
export function generateCustomStrategyBat(config: CustomStrategyConfig): { file: string; content: string } {
  ...
  // В winws режимы называются disorder / fakeddisorder; «diso» и
  // «fakeddiso» он не знает и завершался с ошибкой разбора аргументов.
  const desync =
    config.desyncMode === 'diso' ? 'disorder' : config.desyncMode === 'fakeddiso' ? 'fakeddisorder' : config.desyncMode
...
"%BIN_PATH%winws.exe" --wf-tcp=80,443 --wf-udp=443,50000-65535 ^
  ...
  --new ^
  --filter-tcp=443 --hostlist="%LISTS_PATH%list-general.txt" --dpi-desync=${desync} ${splitArg} ${ttlArg} ${foolingArg} --dpi-desync-fake-tls="%BIN_PATH%${config.fakePayload || 'tls_clienthello_www_google_com.bin'}" ^
