...
const TASK_NAME = 'LazeykaAutoStart'
...
<Task version="1.4" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    <Description>LAZEYKA auto-launch (TG WS Proxy + Zapret)</Description>
  </RegistrationInfo>
  <Triggers>
    <LogonTrigger>
...
export async function enableAutoRun(): Promise<void> {
  if (is.dev) return
  // Снимаем возможную запись в HKCU\Run: автозапуск ведётся через Task
  // Scheduler, и дублирующая запись в реестре запускала бы приложение дважды.
  clearLegacyRunEntry()
  const exe = exePath()
  ...
  const xmlPath = join(tmpdir(), `lazeyka-task-${Date.now()}.xml`)
